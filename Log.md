...
- Xóa dòng `_rp_recalculation_lock = Lock()` còn sót lại sau khi chức năng tính lại RP toàn hệ thống bị loại bỏ ở V1.13.9.
- Dòng này gọi `Lock()` trong khi import `from threading import Lock` đã bị xóa, gây `NameError` ngay lúc Vercel import `app.py`.
- Vì lỗi xảy ra ở cấp module, toàn bộ Serverless Function bị crash và trả `FUNCTION_INVOCATION_FAILED`.
- V1.13.10 kế thừa nguyên lỗi từ V1.13.9 nên cũng bị ảnh hưởng.

## File và vị trí sửa

- `app.py`: khu vực ngay trước `RP_USER_FIELDS` (xấp xỉ dòng 5846 của V1.13.10) – xóa `_rp_recalculation_lock = Lock()`.
- `app.py`: phần khai báo phiên bản – nâng `V1.13.10` lên `V1.13.11`.

## Kiểm tra

- Python compile thành công.
- Không còn tham chiếu `_rp_recalculation_lock` trong `app.py`.
- Không còn lời gọi `Lock()` không được import trong `app.py`.
- Không khôi phục lại chức năng sửa tỷ số hoặc tính lại RP đã chủ động loại bỏ.
- Giữ nguyên tính năng V1.13.10: Trở Lại Phòng Đấu và lịch sử đối đầu trong phiên phòng.
